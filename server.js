...
app.post('/api/lead', async (req, res) => {
  try {
    ...
    const r = await fetch('https://api.brevo.com/v3/contacts', { method: 'POST', headers: { 'api-key': BREVO_API_KEY, 'content-type': 'application/json', 'accept': 'application/json' }, body: JSON.stringify({ email: email.trim().toLowerCase(), attributes: { NOME: nome.trim(), TELEFONE: telefone.trim(), PLACA: placa.trim().toUpperCase(), POSSUI_SEGURO: possui_seguro }, listIds: [BREVO_LIST_ID], updateEnabled: true }) });
    if (!r.ok && r.status !== 204) { const d = await r.text().catch(() => ''); console.error('Erro Brevo:', r.status, d); return res.status(502).json({ ok: false, error: 'Brevo ' + r.status, detail: String(d).slice(0, 300) }); }
